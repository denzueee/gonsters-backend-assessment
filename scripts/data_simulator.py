...
class DataSimulator:
    ...
    def login(self, username: str = "manager1", password: str = "Password123!"):
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/v1/auth/login", json={"username": username, "password": password}, timeout=5
            )
    ...
    def fetch_machines(self, limit: int = 1000):
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/api/v1/data/machines", headers=headers, params={"limit": limit}, timeout=5
            )
            if response.status_code == 200:
                data = response.json()
    ...
    def ingest_data(self):
        ...
        payload = {"gateway_id": self.gateway_id, "timestamp": datetime.utcnow().isoformat() + "Z", "batch": batch}

        try:
            headers = {"Authorization": f"Bearer {self.access_token}", "Content-Type": "application/json"}
            response = requests.post(f"{self.base_url}/api/v1/data/ingest", headers=headers, json=payload, timeout=5)
    ...
    try:
        while True:
            ...
            if success:
                ...
                print(
                    f"[{timestamp}] #{iteration:04d} {anomaly_indicator} "
