...
def get_machine_data(machine_id):  # noqa: C901
    """
    ...
    try:
        ...
        try:
            ...
            return jsonify({"status": "error", "message": "Invalid machine ID format", "machine_id": machine_id}), 400
        ...
        with get_db() as db:
            ...
            if not machine:
                return jsonify({"status": "error", "message": "Machine not found", "machine_id": machine_id}), 404
        ...
        if not start_time_str:
            errors.append({"parameter": "start_time", "error": "Required parameter missing", "value": None})
        else:
            try:
                start_time = datetime.fromisoformat(start_time_str.replace("Z", "+00:00"))
            except ValueError:
                errors.append({"parameter": "start_time", "error": "Invalid ISO 8601 format", "value": start_time_str})
                start_time = None
        ...
        if end_time_str:
            try:
                ...
                errors.append({"parameter": "end_time", "error": "Invalid ISO 8601 format", "value": end_time_str})
        ...
        try:
            limit = int(limit_str)
            # Allow -1 for effectively unlimited (capped at 100k for safety)
            if limit == -1:
                limit = 100000
            elif limit < 1 or limit > 100000:
                errors.append({"parameter": "limit", "error": "Limit must be between 1 and 100000", "value": limit_str})
        except ValueError:
            errors.append({"parameter": "limit", "error": "Limit must be an integer", "value": limit_str})
            limit = 10
        ...
        try:
            ...
            if offset < 0:
                errors.append({"parameter": "offset", "error": "Offset must be non-negative", "value": offset_str})
        except ValueError:
            errors.append({"parameter": "offset", "error": "Offset must be an integer", "value": offset_str})
            offset = 0

        # Kembalikan error jika ada
        if errors:
            return jsonify({"status": "error", "message": "Invalid query parameters", "errors": errors}), 400

        # Query ke InfluxDB
        aggregation_window = None if interval == "raw" else interval

        records = query_sensor_data(
            machine_id=machine_id, start_time=start_time, end_time=end_time, aggregation_window=aggregation_window
        )
        ...
        data_list = sorted(data_points.values(), key=lambda x: x["timestamp"], reverse=True)
    ...
    try:
        ...
        with get_db() as db:
            ...
            new_machine = MachineMetadata(
                name=req_data.name, location=req_data.location, sensor_type=req_data.sensor_type, status=req_data.status
            )
            db.add(new_machine)
            db.commit()
            ...
            return (
                jsonify(
                    {"status": "success", "message": "Machine created successfully", "machine_id": str(new_machine.id)}
                ),
                201,
            )
