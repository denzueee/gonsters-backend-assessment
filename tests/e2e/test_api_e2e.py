...
class TestIngestEndpointE2E:
    ...
    def test_successful_ingestion_single_machine(self, client, auth_headers, sample_machine):
        ...
        response = client.post(
            "/api/v1/data/ingest", data=json.dumps(payload), content_type="application/json", headers=auth_headers
        )
    ...
    def test_successful_ingestion_multiple_machines(self, client, auth_headers, sample_machines):
        ...
        payload = {"gateway_id": "e2e-gateway-002", "timestamp": datetime.utcnow().isoformat() + "Z", "batch": batch}
            ...
            "batch": [
                {
                    ...
                    "location": "Test Floor",
                    "readings": [{"timestamp": datetime.utcnow().isoformat() + "Z", "temperature": 72.5}],
                }
            ],
        }
        ...
        payload = {
            ...
            "batch": [
                ...
                },
                {
                    ...
                    "sensor_type": "Temperature",
                    "location": "Test Floor",
                    "readings": [{"timestamp": datetime.utcnow().isoformat() + "Z", "temperature": 73.0}],
                },
            ],
        }
...
class TestRetrievalEndpointE2E:
    ...
    def test_retrieval_with_aggregation(self, client, auth_headers, app, sample_machine):
        ...
        response = client.get(
            ...
            headers=auth_headers,
            query_string={"start_time": (datetime.utcnow() - timedelta(hours=2)).isoformat() + "Z", "interval": "1h"},
        )
...
class TestCompleteWorkflow:
    ...
    def test_ingest_and_retrieve_workflow(self, client, auth_headers, sample_machine):
            ...
            "batch": [
                {
                    ...
                    "readings": [
                        ...
                        {"timestamp": datetime.utcnow().isoformat() + "Z", "temperature": 73.0, "pressure": 101.5},
                    ],
                }
            ],
        }
        ...
        retrieval_response = client.get(
            f"/api/v1/data/machine/{machine_id}",
            headers=auth_headers,
            query_string={"start_time": (datetime.utcnow() - timedelta(hours=1)).isoformat() + "Z", "interval": "raw"},
        )
    ...
    def test_multiple_gateways_workflow(self, client, auth_headers, sample_machines):
        ...
        machines_data = [
            {"id": str(machine.id), "sensor_type": machine.sensor_type, "location": machine.location}
        ...
        for machine_data in machines_data:
            retrieval_response = client.get(
                ...
                headers=auth_headers,
                query_string={"start_time": (datetime.utcnow() - timedelta(hours=1)).isoformat() + "Z"},
            )
            assert retrieval_response.status_code == 200
