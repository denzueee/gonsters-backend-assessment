...
class TestAuthRoutesAdditional:
    ...
    def test_register_duplicate_username(self, client, db_session):
        ...
        response = client.post(
            "/api/v1/auth/register",
            data=json.dumps(
                {"username": "duplicate", "email": "second@example.com", "password": "Pass123!", "role": "Operator"}
            ),
            content_type="application/json",
        )
    ...
    def test_login_wrong_password(self, client, db_session):
        ...
        response = client.post(
            "/api/v1/auth/login",
            data=json.dumps({"username": "testlogin", "password": "WrongPassword!"}),
            content_type="application/json",
        )
    ...
    def test_login_inactive_user(self, client, db_session):
        ...
        response = client.post(
            "/api/v1/auth/login",
            data=json.dumps({"username": "inactive", "password": "Pass123!"}),
            content_type="application/json",
        )
    ...
    def test_login_nonexistent_user(self, client):
        ...
        response = client.post(
            "/api/v1/auth/login",
            data=json.dumps({"username": "doesnotexist", "password": "Pass123!"}),
            content_type="application/json",
        )
