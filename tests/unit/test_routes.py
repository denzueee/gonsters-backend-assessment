...
class TestIngestEndpoint:
    ...
    def test_ingest_missing_body(self, client, auth_headers):
        ...
        response = client.post("/api/v1/data/ingest", content_type="application/json", headers=auth_headers)
    ...
    def test_ingest_invalid_json(self, client, auth_headers):
        ...
        response = client.post(
            "/api/v1/data/ingest", data="invalid json", content_type="application/json", headers=auth_headers
        )
    ...
    def test_ingest_empty_batch(self, client, auth_headers):
        ...
        payload = {"gateway_id": "test-gateway", "timestamp": datetime.utcnow().isoformat() + "Z", "batch": []}
...
class TestRetrievalEndpoint:
    ...
    def test_retrieval_invalid_interval(self, client, valid_machine_id, auth_headers):
        ...
        response = client.get(
            ...
            query_string={"start_time": datetime.utcnow().isoformat() + "Z", "interval": "invalid_interval"},
            headers=auth_headers,
        )
    ...
    def test_retrieval_invalid_limit(self, client, valid_machine_id, auth_headers):
        ...
        response = client.get(
            f"/api/v1/data/machine/{valid_machine_id}",
            query_string={"start_time": datetime.utcnow().isoformat() + "Z", "limit": "invalid"},
            headers=auth_headers,
        )
    ...
    def test_retrieval_limit_out_of_range(self, client, valid_machine_id, auth_headers):
        ...
        response = client.get(
            f"/api/v1/data/machine/{valid_machine_id}",
            query_string={"start_time": datetime.utcnow().isoformat() + "Z", "limit": 20000},  # Exceeds max 10000
            headers=auth_headers,
        )
