...
from app.schemas import IngestRequest, MachineData, SensorReading
...
class TestSensorReading:
    ...
    def test_valid_sensor_reading(self):
        ...
        reading = SensorReading(timestamp=datetime.utcnow(), temperature=72.5, pressure=101.3, speed=1450.0)
    ...
    def test_partial_sensor_reading(self):
        ...
        reading = SensorReading(timestamp=datetime.utcnow(), temperature=72.5)
    ...
    def test_invalid_temperature_range(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            SensorReading(timestamp=datetime.utcnow(), temperature=-300.0)  # Below absolute zero
    ...
    def test_invalid_pressure_negative(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            SensorReading(timestamp=datetime.utcnow(), pressure=-10.0)
...
class TestMachineData:
    ...
    def test_empty_readings(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            MachineData(machine_id=uuid4(), sensor_type="Temperature", location="Test Floor", readings=[])
...
class TestIngestRequest:
    ...
    def test_empty_batch(self):
        """Test empty batch"""
        with pytest.raises(ValidationError) as exc_info:
            IngestRequest(gateway_id="gateway-001", timestamp=datetime.utcnow(), batch=[])
