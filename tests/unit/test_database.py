...
from app.database import init_influxdb, init_postgres_db, query_sensor_data, write_sensor_data
...
class TestPostgresHandler:
    ...
    def test_create_machine(self, db_session):
        ...
        machine = MachineMetadata(
            name="Unit-Test-Machine", location="Unit Test Floor", sensor_type="Temperature", status="active"
        )
    ...
    def test_query_machine(self, sample_machine, db_session):
        ...
        machine = db_session.query(MachineMetadata).filter(MachineMetadata.id == sample_machine.id).first()
    ...
    def test_update_machine(self, sample_machine, db_session):
        ...
        updated = db_session.query(MachineMetadata).filter(MachineMetadata.id == sample_machine.id).first()
...
class TestInfluxDBHandler:
    ...
    def test_write_sensor_data_partial_fields(self, app, valid_machine_id):
        ...
        with app.app_context():
            # Only temperature
            success = write_sensor_data(
                machine_id=valid_machine_id, sensor_type="Temperature", location="Test Floor", temperature=72.5
            )
            assert success is True

            # Only pressure
            success = write_sensor_data(
                machine_id=valid_machine_id, sensor_type="Pressure", location="Test Floor", pressure=101.3
            )
            assert success is True
